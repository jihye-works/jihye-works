# 👋 → Hye to Hey

**덜어낼수록 선명해지는 경험을 설계합니다.**

불필요한 것을 덜어내고, 사용자에게 더 명확한 경험을 전달하는 기획자 지혜입니다.


## 🧭 About Me
- Planning & Strategy 중심으로 서비스를 설계합니다
- 사용자 흐름과 정보 구조를 중요하게 생각합니다
- "왜 이 기능이 필요한가?"를 끝까지 고민합니다


## 🚀 Skills & Tools

**Planning & Strategy**  
UX Thinking • Service Flow • Figma • Notion  

**Design**  
Photoshop • Illustrator

**Development (Basic)**  
HTML • CSS • JavaScript • React


## 💼 Projects

### 📰 Portfolio Editorial
포트폴리오 기획 및 콘텐츠 구조 설계 (In progress)  
- 사용자 흐름 및 정보 구조 설계  
- 콘텐츠 메시지 기획  
Tech: Figma • HTML  


### 🐈 Cat Diet App
모바일 앱 UI/UX 기획 (In progress)  
- 사용자 문제 정의 및 기능 기획  
- 와이어프레임 설계  
Tech: Figma  


### ☕ Landing Page
랜딩페이지 기획 및 캠페인 소재 제작 (In progress)  
- 전환 중심 구조 설계  
- 비주얼 콘텐츠 기획  
Tech: Figma • Photoshop  


### 🍕 Pizza Website Renewal
기존 웹사이트 리뉴얼 기획 & 퍼블리싱 (In progress)  
- UX 개선 방향 도출  
- 레이아웃 및 인터랙션 구현  
Tech: HTML • CSS • JavaScript  


### 🎬 Short-form Video
영상 콘텐츠 제작 프로젝트 (In progress)  
- 콘텐츠 기획 및 편집  
- 메시지 전달 구조 설계  
Tech: Premiere Pro  




---



## 📫 Contact
- Email: jihyejung.works@gmail.com
- Portfolio: 🔗
- GitHub: 🔗
